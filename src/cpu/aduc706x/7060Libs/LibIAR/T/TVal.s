;Legal: This code is provided as is without warranty.
;	It is the responsibility of the person integrating this
;	code into an application to ensure that the combination
;   performs as required and is safe.
;Status:		Alpha tested.
;Incomplete:	All
;Tools:			Developed using IAR V5.1
;Hardware:		ADuC7060
;Description:	Reads Tn timer value.
;Responsibility:
;	Group:		IAC Software
...
; TVal==========Writes/reads timer T value and starts/stops timer.
; C Function prototype: int TVal(uint uChan, uint uTLd, uint uTGo);
;uChan:{0,1,2,3}
;uTLd:{0-0xffffffff} -> 32bit reload value for T1, 16 bit for all others
;uTGo:{T_STOP,T_RUN,T_RD}
; Description of Function: Reads Tn timer value plus
...
;       >1 or T_RD  -> only read TVAL.
;   Call TVal().	Depending on uTGo stop or runs Tn with uTLd as start
;       value or only reads TnVAL.
;   Returns current TnVAL in r0.
; Robustness: If uChan > 3 then TVal() does nothing and returns uChan unchanged.
...
TVal:	cmp		r0,#3				;if(uChan>3)
		bhi		TVRet				; return uChan;
		mov		r3,#0xff0000		;r3 = Z_BASE
		orr		r3,r3,#0xff000000
		orr		r3,r3,r0,lsl#5		; + uChan*0x20;

		cmp		r2,#1				;if(uTGo>1)
		bhi		TRVal				; only read TnVAL.
...
TRVal:	ldr		r0,[r3,#Z_T0VAL]	;return TnVAL;
TVRet:	bx		lr
