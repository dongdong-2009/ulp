...
void spc_Update(void)
{
	if(((GPIOC->IDR&WLI_0)==WLI_0)&&(i1==1)){
		...
		//nxp_74hct595_WriteByte(&chip,0x5d);                                                               //显示5
			if(a1==1){
			...
			if((time_left(d1)<0)&&b1==1){
				GPIOC->ODR&=~BO_2;                         //准备声消
				GPIOA->ODR&=~BO_CTRL;
				//nxp_74hct595_WriteByte(&chip,0x66);                                        //显示4
				d2=time_get(1000);
				b1=0;
			}
			if((time_left(d2)<0)&&b2==1){
				//nxp_74hct595_WriteByte(&chip,0x4f);                                        //显示3
				d3=time_get(1000);
				b2=0;                                     //不会重复执行
			}
			if((time_left(d3)<0)&&b3==1){
				//nxp_74hct595_WriteByte(&chip,0x5b);                                        //显示2
				d4=time_get(1000);
				b3=0;
			}
			if((time_left(d4)<0)&&b4==1){
				//nxp_74hct595_WriteByte(&chip,0x30);                                       //显示1
				d5=time_get(1000);
				b4=0;
			}
			if((time_left(d5)<0)&&b5==1){
				...
				//nxp_74hct595_WriteByte(&chip,0x71);                                       //显示F
