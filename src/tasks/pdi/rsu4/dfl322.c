...
static const sensor_t rsu_sensor_list[] = {
	{.ecodes = {0x011b}, .grp = 0, -1, MODEL_FRSU, .name = "FRSU", .mark = "98581"},
	{.ecodes = {0x011d}, .grp = 1, -1, MODEL_SRSU, .name = "SRSU", .mark = "98830"},
};

static int ecode_ok_list[] = { //approved ecode list
	0x011a, 0x011b, 0x011d, 0x0182/*req by bruce 2017-09-22*/
};

const static int can_id_tx = 0x752;
const static int can_id_rx = 0x772;
const char *pdi_fixture_id = "R517";
const static int addr_fb = 0xfd80;
...
int pdi_test(int pos, int mask, pdi_report_t *report)
{
	...
	for(int ecu = 0; ecu < 4; ecu ++) {
		...
		if(rsu_index < 0) { //not found :(
			...
			int nfit = fault_search(fb, nread, rsu_sensor_list[rsu_index].ecodes[0]);
